...
[NOTE]
====
Si le certificat devient non valide ultérieurement, l'établissement de liaison TLS échouera. Par conséquent, un message d'erreur est affiché dans le journal d'audit et les messages ne sont plus envoyés au serveur syslog. Pour résoudre ce problème, vous devez corriger le certificat sur le serveur syslog, puis accéder à *menu:Paramètres[Journal d'audit > configurer les serveurs Syslog > tout tester]*.

====
** *Port* -- Entrez le numéro de port du récepteur syslog.
+
Après avoir cliqué sur *Ajouter*, la boîte de dialogue *configurer les serveurs Syslog* s'ouvre et affiche votre serveur syslog configuré sur la page.
